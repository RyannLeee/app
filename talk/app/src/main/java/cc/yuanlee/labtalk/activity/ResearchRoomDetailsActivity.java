...
import cc.yuanlee.easeui.utils.EaseUserUtils;
import cc.yuanlee.easeui.widget.EaseAlertDialog;
import cc.yuanlee.easeui.widget.EaseAlertDialog.AlertDialogUser;
import cc.yuanlee.easeui.widget.EaseExpandGridView;
import cc.yuanlee.labtalk.R;

public class ResearchRoomDetailsActivity extends BaseActivity implements OnClickListener {
	private static final String TAG = "ResearchRoomDetailsActivity";
	...
	public static ResearchRoomDetailsActivity instance;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_research_room_details);
		instance = this;
		st = getResources().getString(R.string.people);
		loadingPB = (ProgressBar) findViewById(R.id.progressBar);

		TextView chatRoomIdTextView = (TextView) findViewById(R.id.tv_conference_room_id_value);
		TextView chatRoomNickTextView = (TextView) findViewById(R.id.tv_conference_room_nick_value);
        ...
        RelativeLayout changeChatRoomDescriptionLayout = (RelativeLayout) findViewById(R.id.rl_change_conference_room_detail);

		// adapter data list
		List<String> ownerAdminList = new ArrayList<String>();
		ownerAdminList.add(room.getOwner());
		ownerAdminList.addAll(room.getAdminList());
		ownerAdminAdapter = new OwnerAdminAdapter(this, R.layout.grid_owner, ownerAdminList);
		...
		membersAdapter = new MemberAdapter(this, R.layout.grid_owner, memberMuteBlockList);
	...
	protected void onActivityResult(int requestCode, int resultCode, final Intent data) {
		...
		String st2 = getResources().getString(R.string.is_quit_the_research_room);
		...
		if (resultCode == RESULT_OK) {
			if (progressDialog == null) {
				progressDialog = new ProgressDialog(ResearchRoomDetailsActivity.this);
					...
					new Thread(new Runnable() {
						...
						public void run() {
							...
							runOnUiThread(new Runnable() {
								@Override
								public void run() {
									TextView tv = (TextView) findViewById(R.id.tv_conference_room_nick_value);
				...
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
                            ...
                            TextView chatRoomTitle = (TextView) findViewById(R.id.tv_conference_room_name);
							...
							Button destroyButton = (Button)ResearchRoomDetailsActivity.this.findViewById(R.id.btn_destroy_conference_room);
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.clear_all_history: // clear conversation history
				...
				new EaseAlertDialog(ResearchRoomDetailsActivity.this, null, st9, null, new AlertDialogUser() {
			...
			case R.id.rl_change_chatroom_name:
				...
				break;
			case R.id.rl_change_conference_room_detail:
	...
	private void showAnnouncementDialog() {
		...
		if(room.getOwner().equals(EMClient.getInstance().getCurrentUser()) ||
			...
			final EditText et = new EditText(ResearchRoomDetailsActivity.this);
				...
				new EMCallBack() {
					...
					public void onError(int code, final String error) {
						runOnUiThread(new Runnable() {
							...
							public void run() {
								progressDialog.dismiss();
								Toast.makeText(ResearchRoomDetailsActivity.this, "update fail," + error, Toast.LENGTH_LONG).show();
							}
						});
					}
	...
	Dialog createMemberMenuDialog() {
		final Dialog dialog = new Dialog(ResearchRoomDetailsActivity.this);
		dialog.setTitle("成员管理");
		dialog.setContentView(R.layout.group_member_menu);
						...
						public void run() {
							try {
                                ...
                                runOnUiThread(new Runnable() {
                                                  @Override
                                                  public void run() {
                                                      Toast.makeText(ResearchRoomDetailsActivity.this, e.getDescription(), Toast.LENGTH_SHORT).show();
                                                  }
                                              }
                                );
								e.printStackTrace();
	...
	private class ChatRoomListener implements EMChatRoomChangeListener {
		...
		public void onMemberJoined(final String roomId, final String participant) {
			if (roomId.equals(ResearchRoomDetailsActivity.this.roomId)) {
				updateRoom();
			}
		}
		...
		public void onMuteListAdded(final String chatRoomId, final List<String> mutes, final long expireTime) {
			if (chatRoomId.equals(ResearchRoomDetailsActivity.this.roomId)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ...
                        Toast.makeText(ResearchRoomDetailsActivity.this, "onMuteListAdded: " + sb.toString(), Toast.LENGTH_SHORT).show();
                    }
                });
				updateRoom();
			}
		...
		public void onMuteListRemoved(final String chatRoomId, final List<String> mutes) {
			if (chatRoomId.equals(ResearchRoomDetailsActivity.this.roomId)) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ResearchRoomDetailsActivity.this, "onMuteListRemoved: " + sb.toString(), Toast.LENGTH_SHORT).show();
                    }
                });
				refreshMembersAdapter();
			}
		...
		public void onAdminAdded(final String chatRoomId, final String admin) {
			if (chatRoomId.equals(ResearchRoomDetailsActivity.this.roomId)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ResearchRoomDetailsActivity.this, "onAdminAdded: " + admin, Toast.LENGTH_SHORT).show();
                    }
                });
				updateRoom();
			}
		...
		public void onAdminRemoved(final String chatRoomId, final String admin) {
			if (chatRoomId.equals(ResearchRoomDetailsActivity.this.roomId)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ResearchRoomDetailsActivity.this, "onAdminRemoved: " + admin, Toast.LENGTH_SHORT).show();
                    }
                });
				updateRoom();
			}
		...
		public void onOwnerChanged(final String chatRoomId, final String newOwner, final String oldOwner) {
			if (chatRoomId.equals(ResearchRoomDetailsActivity.this.roomId)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ResearchRoomDetailsActivity.this, "onOwnerChanged newOwner:" + newOwner + "  oldOwner" + oldOwner, Toast.LENGTH_SHORT).show();
                    }
                });
				updateRoom();
			}
